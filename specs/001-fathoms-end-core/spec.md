...
- **FR-050**: La tienda DEBE ofrecer ítems de exactamente tres categorías: **Armas**, **Armaduras** y **Cartas**.
...
- **Card**: Unidad base del sistema de cartas. Atributos: `id`, `name`, `type: 'action' | 'passive' | 'utility'`, `description`, `rarity`, `effect`, `cooldown?` (solo para acción), `cost: number` (coste en tienda; Carta común 15–25, Carta rara 40–70). Las Armas y Armaduras se modelan como `Card` con `type` adicional `'weapon' | 'armor'` y coste 30–80.
