...
- [ ] T036 [P] [US2] Crear banco de islas JSON con ≥15 islas regulares (incluyendo 2–3 de tipo tienda), 6 islas de jefe y 1 isla final `Fathom's End`; cada isla con `id`, `name`, `type`, `events[]` referenciados por ID en `src/assets/data/islands.json`
...
- [ ] T044 [P] [US3] Crear pool completo de cartas JSON con ≥20 cartas: tipos `action`, `passive`, `utility`, `weapon`, `armor`; rangos de `cost` correctos (común 15–25, rara 40–70, arma/armadura 30–80); efectos `effect: { flatDamage?, damageMult?, flatHp? }` para pasivas en `src/assets/data/cards.json`
...
- [ ] T047 [US3] Actualizar `RewardScreen.vue` para ofrecer selección de cartas del `lootPool` del jefe derrotado (mostrar 1–3 cartas aleatorias, el jugador elige 1, se llama `deckStore.addCard`) en `src/views/RewardScreen.vue` (depende de T030, T043)
